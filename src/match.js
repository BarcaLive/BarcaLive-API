...
export async function handleMatches(isoCode, env, ctx) {
  ...
  const nowNext = getCacheableNow(30);
  const nowPrev = getCacheableNow(300);

  const [nextRes, prevRes] = await Promise.all([
    fetchCached(`${CONFIG.MECZYKI_API}/matches?itemId=${CONFIG.ITEM_ID}&startTime[after]=${nowNext}&limit=5&order[startTime]=asc`, { method: "GET" }, 30, ctx),
    fetchCached(`${CONFIG.MECZYKI_API}/matches?itemId=${CONFIG.ITEM_ID}&startTime[before]=${nowPrev}&limit=15&order[startTime]=desc`, { method: "GET" }, 300, ctx)
  ]);
