...
export async function handleMatches(isoCode, env, ctx) {
  // OPTIMIZATION: Use rounded time for cache keys to increase hit rate
  const nowStringNext = getCacheableNow(30);
  const nowStringPrev = getCacheableNow(300);
  ...
  const [nextRes, prevRes] = await Promise.all([
    fetchCached(`${CONFIG.MECZYKI_API}/matches?itemId=${CONFIG.ITEM_ID}&startTime[after]=${nowStringNext}&limit=5&order[startTime]=asc`, { method: "GET" }, 30, ctx),
