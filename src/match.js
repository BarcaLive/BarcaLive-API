...
export async function handleMatches(isoCode, env, ctx) {
  // Use stable time for cache keys to improve cache hit ratio
  // Both next and prev use the same 5-minute rounded anchor to avoid gaps in coverage
  const stableNow = getCacheableNow(300);
  ...
  const [nextRes, prevRes] = await Promise.all([
    fetchCached(`${CONFIG.MECZYKI_API}/matches?itemId=${CONFIG.ITEM_ID}&startTime[after]=${stableNow}&limit=5&order[startTime]=asc`, { method: "GET" }, 30, ctx),
    fetchCached(`${CONFIG.MECZYKI_API}/matches?itemId=${CONFIG.ITEM_ID}&startTime[before]=${stableNow}&limit=15&order[startTime]=desc`, { method: "GET" }, 300, ctx)
  ]);
