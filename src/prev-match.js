...
export async function handlePrevMatches(iso = 'PL', env, ctx) {
  const nowStr = getRoundedTimeString(new Date(), 300);
  const url = `${CONFIG.MECZYKI_API}/matches?itemId=${CONFIG.ITEM_ID}&startTime[before]=${nowStr}&limit=15&order[startTime]=desc&iso=${iso}`;
