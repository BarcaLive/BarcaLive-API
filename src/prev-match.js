...
export async function handlePrevMatches(iso = 'PL', env, ctx) {
  // OPTIMIZATION: Round timestamps down to match cache TTL (5m).
  ...
  const url = `${CONFIG.MECZYKI_API}/matches?itemId=${CONFIG.ITEM_ID}&startTime[before]=${nowString5m}&limit=15&order[startTime]=desc&iso=${iso}`;
