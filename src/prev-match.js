...
export async function handlePrevMatches(iso = 'PL', env, ctx) {
  const roundedNow = getRoundedTimeString(300);
  const url = `${CONFIG.MECZYKI_API}/matches?itemId=${CONFIG.ITEM_ID}&startTime[before]=${roundedNow}&limit=15&order[startTime]=desc&iso=${iso}`;
